...
<jtalks:hasPermission targetId='${topic.branch.id}' targetType='BRANCH'
  ...
  <a id="move_topic" href="#" class="btn" title="<spring:message code='label.tips.move_topic'/>"
...
<c:if test='${topic.codeReview == null && hasCloseTopicPermission}'>
  <c:choose>
    <c:when test="${topic.closed}">
      <a id="open_topic" href="${pageContext.request.contextPath}/topics/${topic.id}/open" class="btn">
        <spring:message code="label.topic.open"/>
      </a>
    </c:when>
    <c:otherwise>
      <a id="open_topic" href="${pageContext.request.contextPath}/topics/${topic.id}/close" class="btn">
